...
def update_post_reads_per_date_cache(model_id: int, model_type: str, date: datetime.date):
...
def email_author(old: Post, new: Post):
    ...
    print(f'Emailing {author.name} to inform then post ' f'{old.pk} has been deleted.')
    email(author)
